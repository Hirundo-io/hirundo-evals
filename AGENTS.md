...
Core library code lives in `hirundo_evals/`, with behavior-specific helpers collected under `hirundo_evals/evaluation_utils/`. Tests reside in `tests/` and mirror module names (for example `hirundo_evals/evaluation_utils/util_functions.py` ↔ `tests/test_util_functions.py`). Documentation sources are in `docs/`, while runnable examples sit in `examples/`. Distribution artifacts generated by releases land in `dist/`; do not edit them manually.
...
No secrets should live in the repo. Use environment variables for provider tokens and model paths when invoking `run.py`. Verify that large artifacts stay out of version control—`results/` is excluded, so place experimental outputs there and keep them untracked.
